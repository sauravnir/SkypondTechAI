          ...
          >
            ...
            <div className="bg-background p-6 md:p-8 border border-gray-200 font-body">
              ...
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
                <div>
                  <h3 className="text-h3 font-bold text-heading"> Dashboard</h3>
                  <p className="text-small font-body text-muted mt-1">Real-time pharmacy analytics</p>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-green-50 border border-green-200 rounded-lg">
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                  <span className="font-body text-sm text-green-700 font-medium">Live</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {[
                  { icon: Activity, label: "Active Orders", value: "247", change: "+12%", color: "blue" },
                  { icon: CheckCircle2, label: "Completed Today", value: "1,834", change: "+8%", color: "green" },
                  { icon: Clock, label: "Avg. Processing", value: "4.2m", change: "-15%", color: "purple" },
                  { icon: AlertCircle, label: "Pending Review", value: "12", change: "-23%", color: "orange" }
                ].map((stat, idx) => (
                  <motion.div
                    key={stat.label}
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className={`font-body text-xs font-medium ${stat.change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`}>
                        {stat.change}
                      </span>
                    </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-lg font-semibold text-gray-900">Prescription Volume</h4>
                ...
                >
                  ...
                  <div className="space-y-4">
                    {[
                      { icon: CheckCircle2, text: "Order #8472 completed", time: "2m ago", color: "green" },
                      { icon: Package, text: "Inventory restocked", time: "15m ago", color: "blue" },
                      { icon: Users, text: "New patient registered", time: "1h ago", color: "purple" },
                      { icon: AlertCircle, text: "Review required: #8401", time: "2h ago", color: "orange" },
                      { icon: TrendingUp, text: "Daily target achieved", time: "3h ago", color: "green" }
                    ].map((activity, idx) => (
                      <motion.div
                        key={idx}
                        initial={{ opacity: 0, x: 20 }}
              ...
              >
                {[
                  { label: "Error Rate", value: "0.03%", icon: AlertCircle, trend: "down" },
                  { label: "Avg Response Time", value: "1.2s", icon: Clock, trend: "down" },
                  { label: "Customer Satisfaction", value: "98.5%", icon: TrendingUp, trend: "up" }
                ].map((metric, idx) => (
                  <div
                    key={metric.label}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <TrendingUp className={`w-4 h-4 ${metric.trend === 'up' ? 'text-green-600' : 'text-green-600 rotate-180'}`} />
                      </div>
                    </div>
                  </div>
                ))}
